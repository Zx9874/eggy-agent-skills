...
把本仓库的技能按当前宿主和任务域安装到项目，不把官方手册镜像、私人资料或地图运行文件复制进项目。公开包是源码；`docs/official` 只作为仓库参考资料，项目目录中的技能才是安装结果。
...
确认输出包含版本、宿主、档案、技能目录和状态路径；若本次发生写入，再确认备份位置。确认用户已有文档没有被覆盖，官方生成目录没有被写入，地图规则明确标出原点版或世界版。安装完成后打开总工作区并新建会话；当前旧会话已读到的技能不会自动卸载。
