...
当数据存储接口或业务封装提供错误码时，可用该枚举区分成功、参数错误、限流、服务不可用、键或版本不存在等情况，并据此决定重试、降级或提示。

## 使用要点

通过 Enums.DataStoreErrorCode.Success、Enums.DataStoreErrorCode.ReadThrottle、Enums.DataStoreErrorCode.KeyNotFound 等形式引用成员，并与实际获得的错误码进行比较。

## 注意事项

MemoryStore 使用 Enums.MemoryStoreErrorCode，不属于 [DataStoreErrorCode](https://u5-creator.s3.game.163.com/manual/se/game_api/enum/DataStoreErrorCode.html) 范围。普通 DataStore:GetAsync/SetAsync 等公开签名不直接声明返回该枚举；只有在接口或封装明确提供错误码时才比较 [DataStoreErrorCode](https://u5-creator.s3.game.163.com/manual/se/game_api/enum/DataStoreErrorCode.html)。
