# 世界编辑器文档 - 目录

> 官方公开手册快照，抓取日期：2026-08-27。当前官方清单：457 篇可访问页面。
> （版本时间：8.28）。

来源：`https://u5-creator.s3.game.163.com/manual/`
元数据：`https://u5-creator.s3.game.163.com/manual/assets/chunks/metadata.3fab0a39.js`

本公开镜像只收录当前清单中成功获取的页面；旧文件和本地整理稿未纳入。

## 整体目录树

```
世界编辑器文档/
├── 快速上手/
│   ├── 安装指引
│   └── 新手必读
├── 编辑器功能/
│   ├── 蛋仔 AI 编程助手
│   ├── 人形动画制作说明（状态同步版）
│   ├── 场景编辑/
│   │   ├── 主界面介绍
│   │   ├── 外观系统
│   │   ├── 天空盒与滤镜
│   │   ├── 快捷键
│   │   ├── 物理状态的应用
│   │   ├── 画面视角缩放和移动
│   │   ├── 组件
│   │   ├── 运动器
│   │   └── 预设编辑
│   ├── UI编辑器/
│   │   ├── UI 编辑器基础
│   │   └── UI 节点导出与脚本访问
│   ├── 地图周边模块/
│   │   └── 玩法规则
│   └── 自定义文件上传与管理/
│       ├── 自定义文件上传与管理
│       └── 资源包
├── Lua开发教程/
│   ├── SE UGC Lua 作者文档
│   ├── 第 0 章：从可视化编辑器到状态同步 Lua 开发
│   ├── 第 1 章：脚本工程、目录结构与运行入口
│   ├── 第 10 章：数据脚本、配置与资产 URI
│   ├── 第 11 章：数据存储与排行榜
│   ├── 第 12 章：物理、碰撞、Raycast、约束与运动器
│   ├── 第 13 章：角色系统——EggyUnit、HumanUnit 与外观
│   ├── 第 14 章：角色控制与动画——Controller 与 Animator
│   ├── 第 15 章：相机系统——CameraService 与 CameraUnit
│   ├── 第 16 章：音效、特效与场景渲染
│   ├── 第 17 章：资产加载——用 preset 创建模型
│   ├── 第 18 章：自定义资产系统
│   ├── 第 19 章：可复用系统——Manager、生命周期与依赖边界
│   ├── 第 2 章：编辑器对象、属性、坐标与枚举在 Lua 中的表达
│   ├── 第 20 章：平台服务：聊天、跨服、社交、商业化、埋点
│   ├── 第 21 章：测试、调试、日志与 QA
│   ├── 第 22 章：Capstone 综合项目
│   ├── 第 3 章：统一世界、双端模型与逻辑帧
│   ├── 第 4 章：Game 与 Services
│   ├── 第 5 章：Unit 与场景树
│   ├── 第 6 章：事件、Timer 与 Task
│   ├── 第 7 章：Players、输入与角色
│   ├── 第 8 章：客户端与服务端通信
│   └── 第 9 章：UI ／ EUI 与 CoreGui
├── Editor API/
│   ├── 20260806版本更新日志
│   ├── 20260813版本更新日志
│   ├── 20260820版本更新日志
│   ├── 20260827版本更新日志
│   ├── AnimatedUnit
│   ├── AssetService
│   ├── Attachment
│   ├── BackpackItem
│   ├── BallSocketConstraint
│   ├── CameraService
│   ├── CustomDataService
│   ├── DataStore
│   ├── DataStoreErrorCode
│   ├── DataStoreGetOptions
│   ├── DataStoreGetVersionOptions
│   ├── DataStoreIncrementOptions
│   ├── DataStoreInfo
│   ├── DataStoreKeyBriefInfo
│   ├── DataStoreKeyInfo
│   ├── DataStoreListKeyOptions
│   ├── DataStoreListOptions
│   ├── DataStoreOptions
│   ├── DataStoreRemoveOptions
│   ├── DataStoreRemoveVersionOptions
│   ├── DataStoreService
│   ├── DataStoreSetOptions
│   ├── DataStoreType
│   ├── DataStoreVersionInfo
│   ├── EUINode
│   ├── EUIService
│   ├── EUISimpleRichTextLabel
│   ├── Editor API
│   ├── Editor
│   ├── EffectUnit
│   ├── EggyController
│   ├── EggyUnit
│   ├── EventService
│   ├── FindingPathUnit
│   ├── Folder
│   ├── HumanController
│   ├── HumanUnit
│   ├── LinkEffectUnit
│   ├── LocalScript
│   ├── MaterialParam
│   ├── ModuleScript
│   ├── OrderedDataStore
│   ├── OrderedDataStoreKeyValueInfo
│   ├── Pages
│   ├── PathfindingLink
│   ├── PathfindingModifier
│   ├── PhysicsUnit
│   ├── PrefabService
│   ├── PrismaticConstraint
│   ├── PublishService
│   ├── RenderTriggerUnit
│   ├── RenderUnit
│   ├── RigidConstraint
│   ├── RodConstraint
│   ├── Script
│   ├── SelectionService
│   ├── SimpleAreaLight
│   ├── SimpleAreaShadowLight
│   ├── SimpleLineLight
│   ├── SimpleLineShadowLight
│   ├── SimplePointLight
│   ├── SimplePointShadowLight
│   ├── SimpleSpotLight
│   ├── SimpleSpotShadowLight
│   ├── SkinUnit
│   ├── SoundUnit
│   ├── SpaceUnit
│   ├── SpawnLocationUnit
│   ├── Task
│   ├── TexParam
│   ├── TextUnit
│   ├── Tool
│   ├── TriggerUnit
│   ├── Unit
│   ├── World
│   ├── WorldUnit
│   ├── 单位
│   ├── 数据
│   ├── 服务
│   └── 枚举
├── Game API/
│   ├── 数据 Data/
│   │   ├── Animation
│   │   ├── Axes
│   │   ├── BitMask
│   │   ├── BoundingBox
│   │   ├── CFrame
│   │   ├── Camp
│   │   ├── CollisionCallbackInfo
│   │   ├── Color
│   │   ├── Color3
│   │   ├── ColorSequence
│   │   ├── ColorSequenceKeypoint
│   │   ├── CommodityInfo
│   │   ├── Connection
│   │   ├── CustomAppearance
│   │   ├── DataStore
│   │   ├── DataStoreGetOptions
│   │   ├── DataStoreGetVersionOptions
│   │   ├── DataStoreIncrementOptions
│   │   ├── DataStoreInfo
│   │   ├── DataStoreInfoPages
│   │   ├── DataStoreKeyBriefInfo
│   │   ├── DataStoreKeyBriefInfoPages
│   │   ├── DataStoreKeyInfo
│   │   ├── DataStoreListKeyOptions
│   │   ├── DataStoreListOptions
│   │   ├── DataStoreOptions
│   │   ├── DataStoreRemoveOptions
│   │   ├── DataStoreRemoveVersionOptions
│   │   ├── DataStoreSetOptions
│   │   ├── DataStoreVersionInfo
│   │   ├── DataStoreVersionInfoPages
│   │   ├── EUIManager
│   │   ├── EUITouchInfo
│   │   ├── EffectBindData
│   │   ├── EggyAppearance
│   │   ├── Faces
│   │   ├── InputObject
│   │   ├── JoinData
│   │   ├── LogRecord
│   │   ├── MaterialParam
│   │   ├── Matrix
│   │   ├── Matrix3x3
│   │   ├── MemoryStoreHashMap
│   │   ├── MemoryStoreHashMapPages
│   │   ├── MemoryStoreHashMapSetOptions
│   │   ├── MemoryStoreHashMapUpdateOptions
│   │   ├── MemoryStoreQueue
│   │   ├── MemoryStoreQueueAddOptions
│   │   ├── MemoryStoreQueueGetSizeOptions
│   │   ├── MemoryStoreQueueReadOptions
│   │   ├── MemoryStoreSortedMap
│   │   ├── MemoryStoreSortedMapGetRangeOptions
│   │   ├── MemoryStoreSortedMapPages
│   │   ├── MemoryStoreSortedMapSetOptions
│   │   ├── MemoryStoreSortedMapUpdateOptions
│   │   ├── NumberRange
│   │   ├── NumberSequence
│   │   ├── NumberSequenceKeypoint
│   │   ├── OrderedDataStore
│   │   ├── OrderedDataStoreKeyValueInfo
│   │   ├── OrderedDataStoreKeyValueInfoPages
│   │   ├── OverlapParams
│   │   ├── Pages
│   │   ├── PathHitInfo
│   │   ├── PathWaypoint
│   │   ├── PhysicalProperties
│   │   ├── PlayerGui
│   │   ├── Quaternion
│   │   ├── Ray
│   │   ├── RaycastParams
│   │   ├── RaycastResult
│   │   ├── Rect
│   │   ├── Region3
│   │   ├── Region3int16
│   │   ├── RemoteEvent
│   │   ├── ReserveServerResult
│   │   ├── ScreenGui
│   │   ├── Signal
│   │   ├── SubCustomAppearance
│   │   ├── TeleportAsyncResult
│   │   ├── TeleportOptions
│   │   ├── Timer
│   │   ├── Transform
│   │   ├── TriggerCallbackInfo
│   │   ├── TweenInfo
│   │   ├── UDim
│   │   ├── UDim2
│   │   ├── Vector2
│   │   ├── Vector2int16
│   │   ├── Vector3
│   │   ├── Vector3int16
│   │   ├── Vector4
│   │   └── 数据 Data
│   ├── 枚举 Enum/
│   │   ├── AccessoryBindType
│   │   ├── ActivePayerStatus
│   │   ├── ActuatorRelativeTo
│   │   ├── ActuatorType
│   │   ├── AnimationFilterType
│   │   ├── AnimationPriority
│   │   ├── AssetPermission
│   │   ├── AxisType
│   │   ├── BakeMode
│   │   ├── BodyType
│   │   ├── CameraMode
│   │   ├── CameraProjection
│   │   ├── CameraShakeCurve
│   │   ├── CameraShakeType
│   │   ├── CameraType
│   │   ├── CampRelationType
│   │   ├── ChatStyle
│   │   ├── ColorGradingFilter
│   │   ├── ConstraintType
│   │   ├── ConsumptionMode
│   │   ├── ControllerStateType
│   │   ├── CoreGuiType
│   │   ├── DataStoreErrorCode
│   │   ├── DataStoreType
│   │   ├── DefaultCameraPresetType
│   │   ├── DevCameraOcclusionMode
│   │   ├── DevTouchMovementMode
│   │   ├── DisplayDistanceType
│   │   ├── EasingDirection
│   │   ├── EasingStyle
│   │   ├── EffectBindType
│   │   ├── FaceStatus
│   │   ├── FashionPartType
│   │   ├── FieldOfViewMode
│   │   ├── GradientSkyDirection
│   │   ├── HorizontalAlignmentType
│   │   ├── HpBarShowMode
│   │   ├── HumanMeshColor
│   │   ├── KeyCode
│   │   ├── MemoryStoreErrorCode
│   │   ├── MessageType
│   │   ├── NetworkOwnerChangeType
│   │   ├── PathStatus
│   │   ├── PathWaypointAction
│   │   ├── PlayerPlatformSpenderStatus
│   │   ├── PresetLinkStatus
│   │   ├── PresetSkyTemplate
│   │   ├── RaycastFilterType
│   │   ├── SkeletalSocketType
│   │   ├── SkyTemplate
│   │   ├── SortDirection
│   │   ├── TableMajorAxis
│   │   ├── TeleportErrcode
│   │   ├── TeleportState
│   │   ├── TextChatChannelType
│   │   ├── TextFontType
│   │   ├── TextOverflowStrategy
│   │   ├── ThumbnailSize
│   │   ├── ThumbnailType
│   │   ├── TweenPlayState
│   │   ├── TweenStatus
│   │   ├── UserInputState
│   │   ├── UserInputType
│   │   ├── VerticalAlignmentType
│   │   ├── WhenUserFirstPlayed
│   │   └── 枚举 Enum
│   ├── 服务 Service/
│   │   ├── AdvertisementService
│   │   ├── AnalyticsService
│   │   ├── ArchiveService
│   │   ├── AssetService
│   │   ├── CameraService
│   │   ├── Camps
│   │   ├── CommodityService
│   │   ├── ConfigService
│   │   ├── CustomAppearanceService
│   │   ├── DataStoreService
│   │   ├── EffectService
│   │   ├── FriendShipService
│   │   ├── Game
│   │   ├── LightingService
│   │   ├── LogService
│   │   ├── MapData
│   │   ├── MemoryStoreService
│   │   ├── MessageService
│   │   ├── PathfindingService
│   │   ├── PhysicsService
│   │   ├── Players
│   │   ├── ProfileService
│   │   ├── ReplicatedFirst
│   │   ├── ReplicatedStorage
│   │   ├── RunService
│   │   ├── SceneGui
│   │   ├── SocialService
│   │   ├── SoundService
│   │   ├── StarterGui
│   │   ├── StoryService
│   │   ├── TagService
│   │   ├── Task
│   │   ├── TeleportService
│   │   ├── TextChatService
│   │   ├── TimerService
│   │   ├── TweenService
│   │   ├── UserInputService
│   │   ├── VibrationService
│   │   ├── World
│   │   └── 服务 Service
│   ├── 单位 Unit/
│   │   ├── AmbientOcclusionEffect
│   │   ├── AngularMotorUnit
│   │   ├── AnimatedUnit
│   │   ├── AnimationTrack
│   │   ├── Animator
│   │   ├── Attachment
│   │   ├── Backpack
│   │   ├── BackpackItem
│   │   ├── BallSocketConstraint
│   │   ├── BaseController
│   │   ├── BaseMotorUnit
│   │   ├── BasePart
│   │   ├── BasePostEffect
│   │   ├── BaseScript
│   │   ├── BaseSky
│   │   ├── BloomEffect
│   │   ├── CameraUnit
│   │   ├── ClickDetector
│   │   ├── ColorGradingEffect
│   │   ├── ComicEffect
│   │   ├── ConfigSnapshot
│   │   ├── CurveVelMotorUnit
│   │   ├── DepthOfFieldEffect
│   │   ├── EUIButton
│   │   ├── EUIClippingNode
│   │   ├── EUIEffect
│   │   ├── EUIGridLayout
│   │   ├── EUIImage
│   │   ├── EUIInputField
│   │   ├── EUILayout
│   │   ├── EUIListLayout
│   │   ├── EUIListView
│   │   ├── EUILoadingBar
│   │   ├── EUINodeBase
│   │   ├── EUIProgressTimer
│   │   ├── EUIRichTextLabel
│   │   ├── EUIRootNode
│   │   ├── EUISceneNode
│   │   ├── EUISimpleRichTextLabel
│   │   ├── EUITableLayout
│   │   ├── EUITextLabel
│   │   ├── EffectUnit
│   │   ├── EggyController
│   │   ├── EggyUnit
│   │   ├── FileMesh
│   │   ├── FindingPathUnit
│   │   ├── Fog
│   │   ├── Folder
│   │   ├── GradientSky
│   │   ├── HingeConstraint
│   │   ├── HumanController
│   │   ├── HumanUnit
│   │   ├── InkWashEffect
│   │   ├── LensRainEffect
│   │   ├── LinearMotorUnit
│   │   ├── LinkEffectUnit
│   │   ├── LocalScript
│   │   ├── ModelUnit
│   │   ├── ModuleScript
│   │   ├── Path
│   │   ├── PathfindingLink
│   │   ├── PathfindingModifier
│   │   ├── PendulumMotorUnit
│   │   ├── PhysicsUnit
│   │   ├── PixelateEffect
│   │   ├── Player
│   │   ├── PresetLink
│   │   ├── PresetSky
│   │   ├── PrismaticConstraint
│   │   ├── QuadGradientSky
│   │   ├── RenderTriggerUnit
│   │   ├── RenderUnit
│   │   ├── RigidConstraint
│   │   ├── RodConstraint
│   │   ├── Script
│   │   ├── SeatUnit
│   │   ├── SimpleAreaLight
│   │   ├── SimpleAreaShadowLight
│   │   ├── SimpleLight
│   │   ├── SimpleLineLight
│   │   ├── SimpleLineShadowLight
│   │   ├── SimplePointLight
│   │   ├── SimplePointShadowLight
│   │   ├── SimpleSpotLight
│   │   ├── SimpleSpotShadowLight
│   │   ├── SkeletalSocketMount
│   │   ├── SketchEffect
│   │   ├── SkinUnit
│   │   ├── Sky
│   │   ├── SoundGroup
│   │   ├── SoundUnit
│   │   ├── SpaceUnit
│   │   ├── SpawnLocationUnit
│   │   ├── TextUnit
│   │   ├── Tool
│   │   ├── TriggerUnit
│   │   ├── Tween
│   │   ├── Unit
│   │   ├── WaypointMotorUnit
│   │   ├── WeldConstraint
│   │   ├── WorldRoot
│   │   ├── WorldUnit
│   │   └── 单位 Unit
│   ├── 更新日志/
│   │   ├── 2026.07.09版本更新日志
│   │   ├── 2026.07.16版本更新日志
│   │   ├── 2026.07.22版本更新日志
│   │   ├── 2026.07.29版本更新日志
│   │   ├── 2026.08.05版本更新日志
│   │   ├── 2026.08.13版本更新日志
│   │   ├── 2026.08.20版本更新日志
│   │   └── 2026.08.27版本更新日志
│   └── Game API
├── 更新日志/
│   ├── 2026.7.16版本更新说明
│   ├── 2026.7.23版本更新说明
│   ├── 2026.7.30版本更新说明
│   ├── 2026.8.06版本更新说明
│   ├── 2026.8.13版本更新说明
│   ├── 2026.8.20版本更新说明
│   └── 2026.8.27版本更新说明
├── 资产目录/
│   ├── 世界编辑器 · 资产目录：特效
│   ├── 世界编辑器 · 资产目录：组件
│   ├── 世界编辑器 · 资产目录：装饰组件
│   ├── 状态同步编辑器 · 动作 · 资产目录
│   ├── 状态同步编辑器 · 图片 · 资产目录
│   ├── 状态同步编辑器 · 外观散件 · 资产目录
│   ├── 状态同步编辑器 · 散件套装 · 资产目录
│   ├── 状态同步编辑器 · 时装套装 · 资产目录
│   ├── 状态同步编辑器 · 时装配饰 · 资产目录
│   ├── 状态同步编辑器 · 模型 · 资产目录
│   └── 状态同步编辑器 · 音频 · 资产目录
├── 多人协作/
│   └── 多人协作
└── 编辑器插件开发教程/
    └── 编辑器插件开发教程
└── 目录.md
```

## 抓取结果

- 成功：457
- 失败：0

> 其中“蛋仔 AI 编程助手”和“人形动画制作说明（状态同步版）”存在于官方静态资源清单，但未挂在侧边栏，已一并收录。

- 旧文件未发布数：21
